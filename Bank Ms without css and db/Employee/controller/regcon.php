<?php
    ...
    $fname = $uname = $email = $phoneNumber =$gender =$password = $cpassword =$address =$pic =$field =$education="";
	$flag=0;
		if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            /*$user =['username'=> $uname, 'fname'=>$fname, 'password'=>$password, 'email'=>$email,'phone' =>$phoneNumber,'Address' =>$Address,'gender' =>$gender,'field' =>$field,'education' =>$education,'type' =>$type];
			$_SESSION['user'] = $user;*/
    }
	else{
...
header("refresh: 2; url = ../view/signup.php");
    ...
    $txt = $uname."|".$password."|".$fname."|".$email."|".$phoneNumber."|".$gender ."|".$address."|".$education."|".$field."\n";
    fwrite($myfile, $txt);
    fclose($myfile);
